...
public class GrindStone : MonoBehaviour, Hoverable, Interactable
{
    ...
    public static IEnumerator GiveSharpness(StatusEffect[] effects, int time)
    {
        ...
        //Console.instance.Print(weapon.m_shared.m_skillType.ToString());
        int number = 1;
        ...
        if (weapon.m_shared.m_skillType.ToString() == "Knives")
        {
            number = 2;
        }
        ...
        if (weapon.m_shared.m_skillType.ToString() != "Axes"
           ...
           && weapon.m_shared.m_skillType.ToString() != "Knives"
    ...
    {
        ZInput.ButtonDef key;
        ZInput.m_instance.m_buttons.TryGetValue("Use", out key);
        return "\n[<color=yellow><b>"+key.m_key.ToString()+"</b></color>] Sharpen your weapon";
    }
    ...
    public bool Interact(Humanoid user, bool hold)
    {
        ...
        if(user.m_rightItem.m_shared.m_equipStatusEffect != null)
        {
            Console.instance.Print("This Item already has a status effect on it");
            return false;
        }
